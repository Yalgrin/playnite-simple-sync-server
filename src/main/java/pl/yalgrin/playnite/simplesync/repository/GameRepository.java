...
public interface GameRepository extends ObjectRepository<Game> {
    ...
    @Query("select g.* from playnite_game g where g.game_id = :gameId and g.plugin_id = :pluginId order by g.id")
    Flux<Game> findByGameIdAndPluginId(String gameId, String pluginId);

    @Query("select g.* from playnite_game g where g.playnite_id = :playniteId and g.game_id = :gameId and g.plugin_id = :pluginId and g.removed = false order by g.id")
