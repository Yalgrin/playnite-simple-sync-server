...
class GameDiffV2MigratorTest extends SpockIntegrationTest {
...
"""
    ...
    @Autowired
    private GameDiffRepository repository
    @Autowired
    private JsonMapper jsonMapper
    @Autowired
    private GameDiffV2Migrator migrator
    ...
    def "should migrate JSON properly"() {
        ...
        StepVerifier.create(repository.findById(entity.id))
                .expectNextMatches {
                    ...
                    assert node.get("GameDiffId") == null
                    ...
                    assert changedFields.size() == 2
                    assert changedFields.get(0)?.stringValue() == "Name"
                    assert changedFields.get(1)?.stringValue() == "Platforms"
