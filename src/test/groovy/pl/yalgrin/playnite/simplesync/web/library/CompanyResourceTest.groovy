...
import pl.yalgrin.playnite.simplesync.util.library.CompanyAssertionUtil
import pl.yalgrin.playnite.simplesync.util.library.CompanyFactoryUtil
...
class CompanyResourceTest extends AbstractObjectTest<Company, CompanyDTO> {
    ...
    def "save company and then delete it"() {
        ...
        StepVerifier.create(IntegrationTestUtil.getReturnMono(saveResponse, CompanyDTO.class))
                .expectNextMatches {
                    assert it.serverId != null
                    newObjectId.set(it.serverId)
                    objectMatches(it, dto)
    ...
    def "save, modify and delete and await the change stream"() {
        ...
        CompanyDTO modified = toSave.withName("some other name")
        CompanyDTO removed = modified.withRemoved(true)
        ...
        StepVerifier.create(responseFlux)
                ...
                .then {
                    def result = makeSaveRequest(toSave).expectStatus().is2xxSuccessful()
                            ...
                            .returnResult()
                            .responseBody
                    CompanyAssertionUtil.assertCompany(toSave, result)
                ...
                .then {
                    def result = makeSaveRequest(modified).expectStatus().is2xxSuccessful()
                            ...
                            .returnResult()
                            .responseBody
                    CompanyAssertionUtil.assertCompany(modified, result)
