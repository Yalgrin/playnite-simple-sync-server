...
import pl.yalgrin.playnite.simplesync.util.library.SourceAssertionUtil
import pl.yalgrin.playnite.simplesync.util.library.SourceFactoryUtil
...
class SourceResourceTest extends AbstractObjectTest<Source, SourceDTO> {
    ...
    def "save source and then delete it"() {
        ...
        StepVerifier.create(IntegrationTestUtil.getReturnMono(saveResponse, SourceDTO.class))
                .expectNextMatches {
                    assert it.serverId != null
                    assert it.createdAt != null
                    assert it.createdBy != null
    ...
    def "save, modify and delete and await the change stream"() {
        ...
        SourceDTO modified = toSave.withName("some other name")
        SourceDTO removed = modified.withRemoved(true)
        ...
        StepVerifier.create(responseFlux)
                ...
                .then {
                    def result = makeSaveRequest(toSave).expectStatus().is2xxSuccessful()
                            .expectBody(SourceDTO.class)
                            .returnResult()
                            .responseBody
                    SourceAssertionUtil.assertSource(toSave, result)
                ...
                .then {
                    def result = makeSaveRequest(modified).expectStatus().is2xxSuccessful()
                            ...
                            .returnResult()
                            .responseBody
                    SourceAssertionUtil.assertSource(modified, result)
